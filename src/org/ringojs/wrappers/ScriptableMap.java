...
/**
 * ScriptableMap is a wrapper for java.util.Map instances that allows developers to interact with
 * them as if it were a native JavaScript object.
 */
public class ScriptableMap extends NativeJavaObject {
  ...
  public static void init(Scriptable scope) throws NoSuchMethodException {
    BaseFunction ctor = new BaseFunction(scope, ScriptableObject.getFunctionPrototype(scope)) {
      @Override public Scriptable construct(Context cx, Scriptable scope, Object[] args) {
        ...
        if (args.length > 2) { throw new EvaluatorException(
            "ScriptableMap() called with too many arguments"); }
        if (args.length == 2) {
  ...
  @SuppressWarnings("unchecked") private ScriptableMap(Scriptable scope, Object obj, boolean reflect) {
    this.parent = scope;
  ...
  public Object get(int index, Scriptable start) {
    if (map == null) { return super.get(index, start); }
    return getInternal(new Integer(index));
  }
  private Object getInternal(Object key) {
    Object value = map.get(key);
    if (value == null) { return Scriptable.NOT_FOUND; }
  ...
  @SuppressWarnings("unchecked") private void putInternal(Object key, Object value) {
    try {
  ...
  public String toString() {
    if (map == null) return super.toString();
    return map.toString();
  }
