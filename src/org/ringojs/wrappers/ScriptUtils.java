...
public class ScriptUtils {
  ...
  /**
   * Unwrap a JS object to a java object. This is much more conservative than Context.jsToJava in
   * that it will preserve undefined values.
  ...
  public static void checkArguments(Object[] args, int min, int max) {
    if (min > -1 && args.length < min) throw new IllegalArgumentException();
    if (max > -1 && args.length > max) throw new IllegalArgumentException();
  }
  /**
  ...
  /**
   * Get a snapshot of the current JavaScript evaluation state by creating an Error object and
   * invoke the function on it passing along any arguments. Used to invoke console.trace() and
   * friends because implementing this in JavaScript would mess with the evaluation state.
