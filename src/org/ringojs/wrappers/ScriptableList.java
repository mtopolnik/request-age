...
/**
 * ScriptableList is a wrapper for java.util.List instances that allows developers to interact with
 * them like it was a native JavaScript array.
 */
public class ScriptableList extends NativeJavaObject {
  ...
  public static void init(Scriptable scope) throws NoSuchMethodException {
    BaseFunction ctor = new BaseFunction(scope, ScriptableObject.getFunctionPrototype(scope)) {
      @Override public Scriptable construct(Context cx, Scriptable scope, Object[] args) {
        if (args.length > 1) { throw new EvaluatorException(
            "ScriptableList() requires a java.util.List argument"); }
    ...
    ScriptableObject.defineProperty(scope, CLASSNAME, ctor, ScriptableObject.DONTENUM
   ...
   */
  @SuppressWarnings("unchecked") private ScriptableList(Scriptable scope, Object obj) {
    this.parent = scope;
   ...
   */
  @SuppressWarnings("unchecked") public ScriptableList(Scriptable scope, List list) {
  ...
  public Object get(int index, Scriptable start) {
    if (list == null) return super.get(index, start);
    try {
  ...
  public boolean has(int index, Scriptable start) {
    if (list == null) return super.has(index, start);
  ...
  public Object get(String name, Scriptable start) {
    if ("length".equals(name) && list != null) { return new Integer(list.size()); }
    return super.get(name, start);
  }
  public Object[] getIds() {
    if (list == null) return super.getIds();
    int size = list.size();
